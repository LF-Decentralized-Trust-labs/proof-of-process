# Todo
<!-- suggest | Updated: 2026-03-17 | Domain: mixed | Languages: rust | Content: specification, cicd, documentation | Files: 36 | Issues: 48 -->
...
## Systemic Issues
- [ ] **SYS-001** `unpinned_action` — 6 workflows — HIGH
...
- [ ] **SYS-002** `draft_naming_stale` — 5 files — HIGH
...
- [ ] **SYS-003** `missing_permissions_block` — 4 workflows — MEDIUM
...
## Critical
- [ ] **C-001** `[correctness]` `draft-condrey-cpop-appraisal.md:1452` — Tool receipt PKI deferred to unspecified companion document
...
- [ ] **C-002** `[correctness]` `draft-condrey-cpop-appraisal.md:617` — Verdict contradiction resolution undefined
...
- [ ] **C-003** `[correctness]` `draft-condrey-cpop-appraisal.md:598` — Perplexity scoring has conflicting normative language (SHOULD compute vs MUST flag)
...
- [ ] **C-004** `[correctness]` `draft-condrey-cpop-protocol.md:1337` — attestation-tier is optional (?) but Verifier MUST assess tier from it
...
- [ ] **C-005** `[correctness]` `examples/writers-authenticity-report.cddl-diag:76` — COSE_Sign1 placeholder (4 bytes) invalid against schema
...
## High
- [ ] **H-001** `[correctness]` `draft-condrey-cpop-appraisal.md:577` — SNR spectral flatness algorithm undefined
...
- [ ] **H-002** `[correctness]` `draft-condrey-cpop-appraisal.md:580` — CLC "semantic complexity" undefined and non-deterministic
...
- [ ] **H-003** `[correctness]` `draft-condrey-cpop-appraisal.md:583` — C_intra (Mechanical Turk) "pause duration" and "edit complexity" undefined
...
- [ ] **H-004** `[correctness]` `draft-condrey-cpop-appraisal.md:475` — Step 7 (Channel Binding) verdict assignment missing
...
- [ ] **H-005** `[completeness]` `draft-condrey-cpop-appraisal.md:493` — Profile mismatch fallback missing
...
- [ ] **H-006** `[completeness]` `draft-condrey-cpop-appraisal.md:1573` — Assistive mode spoofing risk
...
- [ ] **H-007** `[completeness]` `draft-condrey-cpop-appraisal.md:1387` — Effort attribution for CORE cross-references undefined
...
- [ ] **H-008** `[structure]` `draft-condrey-cpop-appraisal.md:1010` — "windows" in forensic-flag CDDL never defined
...
- [ ] **H-009** `[completeness]` `draft-condrey-cpop-protocol.md:1630` — Tool receipt key discovery mechanism unspecified
...
- [ ] **H-010** `[completeness]` `draft-condrey-cpop-protocol.md:1150` — Attester state machine section is stub
...
- [ ] **H-011** `[correctness]` `draft-condrey-cpop-protocol.md:944` — identity-fingerprint input undefined
...
- [ ] **H-012** `[correctness]` `draft-condrey-cpop-protocol.md:2791` — Threat model incomplete (Dolev-Yao defined but not used)
...
- [ ] **H-013** `[correctness]` `.github/workflows/ghpages.yml:21` — Missing `pages: write` permission
...
- [ ] **H-014** `[correctness]` `.github/workflows/publish.yml:28` — Unquoted `github.ref` in git fetch command
...
- [ ] **H-015** `[best_practices]` `.github/workflows/ghpages.yml:33` — Timestamp-based cache key rebuilds daily
...
- [ ] **H-016** `[completeness]` `draft-condrey-cpop-appraisal.md:1997` — T4 entropy trajectory SD undefined
...
- [ ] **H-017** `[correctness]` `draft-condrey-cpop-appraisal.md:483` — Reference hardware definition imprecise
...
- [ ] **H-018** `[completeness]` `draft-condrey-cpop-appraisal.md:1259` — Baseline z-score threshold only in table, not normative text
...
## Medium
- [ ] **M-001** `[correctness]` `draft-condrey-cpop-protocol.md:870` — Checkpoint interval SHOULD vs MUST ambiguity
- [ ] **M-002** `[correctness]` `draft-condrey-cpop-protocol.md:1547` — pop-timestamp allows zero (should be `.gt 0`)
- [ ] **M-003** `[correctness]` `draft-condrey-cpop-protocol.md:1250` — Unknown field handling: ignore-all vs strict-reserved-range
- [ ] **M-004** `[correctness]` `draft-condrey-cpop-protocol.md:877` — Packet rollover trigger conditions undefined
- [ ] **M-005** `[correctness]` `draft-condrey-cpop-protocol.md:1678` — Edit-graph array truncation "recent" ambiguous
- [ ] **M-006** `[correctness]` `draft-condrey-cpop-protocol.md:2087` — Nonce vs beacon ordering in seed undefined
- [ ] **M-007** `[correctness]` `draft-condrey-cpop-protocol.md:3003` — Key rotation grace period unspecified
- [ ] **M-008** `[correctness]` `draft-condrey-cpop-protocol.md:2314` — Beacon fetch timeout unspecified
- [ ] **M-009** `[correctness]` `draft-condrey-cpop-protocol.md:936` — Baseline identity-fingerprint mismatch not checked
- [ ] **M-010** `[correctness]` `draft-condrey-cpop-protocol.md:3100` — Jitter quantization default (5ms) not enforced
- [ ] **M-011** `[consistency]` `draft-condrey-cpop-appraisal.md:454` — CLC terminology inconsistent across document
- [ ] **M-012** `[completeness]` `draft-condrey-cpop-appraisal.md:1203` — Entropy units ambiguous (per-sample vs aggregate)
- [ ] **M-013** `[completeness]` `draft-condrey-cpop-appraisal.md:1279` — Baseline similarity thresholds arbitrary, no sensitivity analysis
- [ ] **M-014** `[completeness]` `draft-condrey-cpop-appraisal.md:949` — Absence proof priority vs forensic flags undefined
- [ ] **M-015** `[completeness]` `draft-condrey-cpop-appraisal.md:1268` — Baseline flag status unclear (forensic flag or not?)
- [ ] **M-016** `[structure]` `draft-condrey-cpop-appraisal.md:1856` — Normative content in unnumbered appendix
- [ ] **M-017** `[correctness]` `ROADMAP.md:16` — Crate versions stale (v0.2.0→0.2.1, v0.1.0→0.1.1)
- [ ] **M-018** `[consistency]` `cddl/cpop.cddl:224` — presence-challenge COSE_Sign1 lacks `.cbor` constraint
- [ ] **M-019** `[consistency]` `cddl/cpop.cddl:1` — COSE_Sign1 external dependency not declared
- [ ] **M-020** `[completeness]` `examples/evidence-packet.cddl-diag:4` — 6 optional fields omitted from example
- [ ] **M-021** `[consistency]` `README.md:162` — Mixed license file references (LICENSE vs LICENSE.md)
- [ ] **M-022** `[security]` `.github/workflows/release.yml:27` — Redundant GITHUB_TOKEN injection
- [ ] **M-023** `[consistency]` `.github/workflows/rust.yml:28` — Inconsistent cache key strategy across workflows
- [ ] **M-024** `[completeness]` `CONTRIBUTING.md:53` — Missing Rust toolchain setup for crate contributors
- [ ] **M-025** `[correctness]` `draft-condrey-cpop-protocol.md:2530` — Security bound per-checkpoint vs per-session ambiguous
