...
/// Backward-compat alias for `CBOR_TAG_CPOP` (v0.1 name).
pub const CBOR_TAG_EVIDENCE_PACKET: u64 = CBOR_TAG_CPOP;

/// Backward-compat alias for `CBOR_TAG_CWAR` (v0.1 name).
...
// === v0.1 backward-compatible functions using crate::error::Error ===
...
pub fn encode_evidence(packet: &crate::rfc::EvidencePacket) -> crate::error::Result<Vec<u8>> {
    ...
    ciborium::ser::into_writer(
        &Required::<&crate::rfc::EvidencePacket, CBOR_TAG_CPOP>(packet),
        &mut bytes,
    )
...
pub fn decode_evidence(bytes: &[u8]) -> crate::error::Result<crate::rfc::EvidencePacket> {
    let tagged: ciborium::tag::Required<crate::rfc::EvidencePacket, CBOR_TAG_CPOP> =
...
pub fn encode_attestation(result: &crate::rfc::AttestationResult) -> crate::error::Result<Vec<u8>> {
    ...
    let mut bytes = Vec::new();
    ciborium::ser::into_writer(
        &Required::<&crate::rfc::AttestationResult, CBOR_TAG_CWAR>(result),
        &mut bytes,
    )
    .map_err(|e| crate::error::Error::Serialization(e.to_string()))?;
    Ok(bytes)
...
pub fn decode_attestation(bytes: &[u8]) -> crate::error::Result<crate::rfc::AttestationResult> {
    let tagged: ciborium::tag::Required<crate::rfc::AttestationResult, CBOR_TAG_CWAR> =
        ciborium::de::from_reader(bytes)
            .map_err(|e| crate::error::Error::Serialization(e.to_string()))?;
    Ok(tagged.0)
}
...
mod tests {
    ...
    fn test_backward_compat_aliases() {
        assert_eq!(CBOR_TAG_EVIDENCE_PACKET, CBOR_TAG_CPOP);
        assert_eq!(CBOR_TAG_ATTESTATION_RESULT, CBOR_TAG_CWAR);
    }
}
